...
import { menuService } from "@/app/home/leftSideBar/services/menuService";
import {
  buildMenuTree,
  ensureBuiltInMenus,
  filterMenusByCategory,
} from "@/app/home/leftSideBar/utils/menuUtils";
  ...
  const login = async (credentials: LoginRequest) => {
    ...
    try {
      ...
      appStore.pageDomain.home.uiDomain.layout.leftSidebar.menu.platform =
        buildMenuTree(filterMenusByCategory(menus, 1));
      appStore.pageDomain.home.uiDomain.layout.leftSidebar.menu.project =
        buildMenuTree(filterMenusByCategory(menus, 2));

      setPhase("redirecting");
      window.location.replace(toBrowserAppPath("/home"));
