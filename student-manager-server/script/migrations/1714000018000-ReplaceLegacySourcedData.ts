...
export class ReplaceLegacySourcedData1714000018000
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    for (const [tableName, columns] of Object.entries(this.businessTables)) {
      await this.replaceTableFromLegacy(queryRunner, tableName, columns);
    }

    await this.replaceFileStorageFromLegacy(queryRunner);
    await this.replaceLegacyMemberStyleFromSysUser(queryRunner);
  }

  public async down(): Promise<void> {
    // Intentionally no-op. The previous target rows are replaced by liuxie data
    // and cannot be safely reconstructed by a migration rollback.
  }

  private async replaceTableFromLegacy(
    ...
    await queryRunner.query(`DELETE FROM ${this.quote(tableName)}`);
  ...
  private async replaceFileStorageFromLegacy(
    queryRunner: QueryRunner,
  ): Promise<void> {
    ...
    await queryRunner.query('DELETE FROM `file_storage`');
  ...
  private async replaceLegacyMemberStyleFromSysUser(
    queryRunner: QueryRunner,
  ): Promise<void> {
