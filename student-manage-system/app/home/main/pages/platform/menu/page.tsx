...
export default function MenuManagePage() {
  ...
  useEffect(() => {
    ...
    menuApi.getAllMenus().then(setAllMenus).catch(console.error)
  }, [currentPage, pageSize])

  const flattenTree = (menus: MenuData[], depth = 0): Array<MenuData & { depth: number }> => {
  ...
  const renderCell = (item: MenuData, columnKey: string) => {
    ...
    switch (columnKey) {
      case "name":
        return (
          <div style={{ paddingLeft: `${((item as MenuData & { depth?: number }).depth || 0) * 1.5}rem` }} className="flex items-center gap-2">
            {((item as MenuData & { depth?: number }).depth || 0) > 0 && <span className="text-gray-300">├</span>}
            {item.name}
          </div>
        )

      case "type":
        const typeInfo = menuTypeChipMap[String(item.type)] || { label: "未知", color: "default" as const }
        return (
          <Chip
      ...
      <section className="rounded-[28px] border border-white/70 bg-[linear-gradient(135deg,_rgba(14,165,233,0.12),_rgba(255,255,255,0.95)_45%,_rgba(16,185,129,0.12))] p-6 shadow-[0_25px_70px_-45px_rgba(15,23,42,0.45)]">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-3 text-3xl font-semibold text-slate-900">菜单管理</h1>
            <p className="mt-2 text-sm leading-6 text-slate-600">管理系统内的功能菜单、权限控制和侧边栏展现。</p>
          </div>
          <Button
            color="primary"
      ...
      <Modal isOpen={isModalOpen} onOpenChange={setIsModalOpen} size="3xl" scrollBehavior="inside">
        <ModalContent>
          ...
          <ModalBody className="grid gap-4 md:grid-cols-2">
            <Input label="菜单名称" value={formState.name || ""} onChange={(e) => setFormState((prev) => ({ ...prev, name: e.target.value }))} />
            <Input label="菜单编码" value={formState.code || ""} onChange={(e) => setFormState((prev) => ({ ...prev, code: e.target.value }))} />
            <Select
              label="菜单类型"
              selectedKeys={[String(formState.type ?? 2)]}
              onSelectionChange={(keys) => setFormState((prev) => ({ ...prev, type: Number(Array.from(keys)[0] || 2) }))}
            >
            ...
            <Select
              ...
              onSelectionChange={(keys) => setFormState((prev) => ({ ...prev, category: Number(Array.from(keys)[0] || 1) }))}
            >
              {menuCategoryOptions.map((option) => (
                <SelectItem key={option.value}>{option.label}</SelectItem>
              ))}
            </Select>
            <Input label="图标" value={formState.icon || ""} onChange={(e) => setFormState((prev) => ({ ...prev, icon: e.target.value }))} />
            <Input label="组件路径" value={formState.component || ""} onChange={(e) => setFormState((prev) => ({ ...prev, component: e.target.value }))} />
            <Input label="菜单路径" value={formState.path || ""} onChange={(e) => setFormState((prev) => ({ ...prev, path: e.target.value }))} />
            <Input label="权限标识" value={formState.permission || ""} onChange={(e) => setFormState((prev) => ({ ...prev, permission: e.target.value }))} />
            <Select
              label="父菜单"
              selectedKeys={formState.parentId === null || formState.parentId === undefined ? ["none"] : [String(formState.parentId)]}
              onSelectionChange={(keys) => {
                const parentId = Array.from(keys)[0]
                setFormState((prev) => ({
                  ...prev,
                  parentId: parentId === "none" || parentId === undefined ? null : Number(parentId),
                }))
              }}
            >
              {[
